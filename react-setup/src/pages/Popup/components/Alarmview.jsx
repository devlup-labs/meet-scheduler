    ...
    let data = await getAllDataFromStorage();
      ...
      var alarmCodeCourse = {};
			for (var key in alarmAll) {
				if(!alarmCodeWise[alarmAll[key].code]) alarmCodeWise[alarmAll[key].code] = [];
      ...
      {this.state.alarms.map((alarm) => {
        return (
          <ListItem key={alarm.code}>
            ...
            <ListItemText id={alarm.code} primary={`${alarm.course}`} secondary={`${alarm.code}`} />
            <ListItemSecondaryAction>
              <IconButton edge="end" aria-label="delete" onClick={() => {this.DeleteAlarm(alarm.code)}}>
